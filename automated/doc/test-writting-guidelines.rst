...
On build like OpenEmbedded, there isn't an easy way to install packages, even
compile and install from source is impossible when tool chain isn't available.
The required dependencies should be pre-installed in the build. To run test case
that contain install steps on this kind of build:

    * Define 'SKIP_INSTALL' variable with 'False' as default.
    * Add parameter '-s <True|False>', so that user can modify 'SKIP_INSTALL'.
    * For package install, "install_dpes ${pkgs} ${SKIP_INSTALL}" will check
      the value of 'SKIP_INSTALL' to determine whether skip the install.
    * When you have customized install steps like code downloading, compilation
      and install defined, you will needed to do the check yourself.
...
sending to LAVA. The following result format should be followed.
    ...
    test-case--d pass/fail/skip measurement units
...
A general test definition file should contain the below keywords and steps::
    ...
    params:
      SKIP_INSTALL: False
    run:
      # A typical tet run in LAVA requires the below steps.
      steps:
...
A typical Android test case can be wrote with the following steps::
...
* Check test case with the following tools if applicable.
...
* Run test case on local system without LAVA.
* Run test case in LAVA and provide job example.
