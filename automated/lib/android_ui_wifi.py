...
def set_wifi_state(dut, turn_on):
    ...
    wifi_disabler = dut(text='ON',
        resourceId='com.android.settings:id/switch_widget')

    if not wifi_enabler.exists and not wifi_disabler.exists:
        raise DeviceCommandError(dut, 'UI: set Wi-Fi state',
...
def main():
    ...
    parser.add_argument('-a', dest='ACTION', required=True, nargs='+',
                        help="Action to perform. Following action is currently implemented: \
                        set_wifi_state <on|off>")
    parser.add_argument('-s', dest='SERIALS', nargs='+',
                        help="Serial numbers of devices to configure. \
                        If not present, all available devices will be configured.")
    args = parser.parse_args()

    if (args.ACTION[0] != 'set_wifi_state'
        or args.ACTION[1] not in ('on', 'off')):
        print('ERROR: Specified ACTION is not supported: {}'.format(args.ACTION), file=sys.stderr)
        sys.exit(1)
