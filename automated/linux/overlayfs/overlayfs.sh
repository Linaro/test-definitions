...
install_deps "git gzip"

# Check kernel config CONFIG_OVERLAY_FS enabled.
if [ -f "/proc/config.gz" ]; then
    check_cmd="gunzip -c /proc/config.gz | grep CONFIG_OVERLAY_FS=y"
elif [ -f "/boot/config-$(uname -r)" ]; then
    check_cmd="grep CONFIG_OVERLAY_FS=y /boot/config-$(uname -r)"
fi
[ -n "${check_cmd}" ] && exit_on_fail "check-kernel-config"
