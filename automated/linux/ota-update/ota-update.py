...
if match_sha_on_server(installed_sha) == 0:
  r=requests.put(url, data=data, headers=headers)
  if match_sha_on_server(update_sha) == 0:
    print "PASS: %s updated to %s successfully" % devicename, update_sha
  else:
    print "FAIL: %s update to %s failed" % devicename, update_sha
else:
  print "FAIL: %s update to %s failed" % devicename, update_sha
