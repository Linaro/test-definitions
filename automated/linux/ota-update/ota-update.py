...
if match_sha_on_server(args.installed_sha) == 0:
    ...
    if match_sha_on_server(args.update_sha) == 0:
        py_test_lib.add_result(RESULT_FILE, "ota-update pass")
        print "PASS: %s updated to %s successfully" % (args.devicename, args.update_sha)
    else:
        py_test_lib.add_result(RESULT_FILE, "ota-update fail")
    ...
    print "FAIL: Insalled device sha to %s mismatched on the server" % (args.devicename, args.update_sha)
