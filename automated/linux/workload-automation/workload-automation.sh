...
usage() {
    echo "Usage: $0 [-s <true|false>] [-t <wa_tag>] [-r <wa_templates_repo>] [-c <config>] [-a <agenda>]" 1>&2
    exit 1
}

while getopts ":s:t:r:c:a:" opt; do
    case "${opt}" in
...
# Setup SSH login.
if grep "host='lava-target-ip'" ./config.py; then
    info_msg "Please run setup-remote-target.yaml on remote target."
    target_ip=$(lava-target-ip)
    sed -i "s/lava-target-ip/${target_ip}/" ./config.py
elif grep "host='127.0.0.1'" ./config.py; then
    SKIP_INSTALL="true"
    "./setup-target.sh" "${SKIP_INSTALL}"
fi
