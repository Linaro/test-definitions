...
fio_rw() {
    ...
    parse_output "fio-${block_size}-rw" "${file}"
    rm -rf ./rw*
}

fio_randrw() {
    block_size="$1"
    file="${OUTPUT}/fio-${block_size}-randrw.txt"

    info_msg "Running fio ${block_size} randrw test..."
    fio -name=randrw -rw=randrw -bs="${block_size}" -size=1G -runtime=300 \
        -ioengine="${IOENGINE}" -direct=1 -group_reporting -output="${file}"
    echo

    parse_output "fio-${block_size}-randrw" "${file}"
    rm -rf ./randrw*
}
