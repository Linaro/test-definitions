...
# tmp log
git checkout fix-remote-tests

pip3 install -r requirements.txt

export SPIRE_PAT_TOKEN LAVA_TOKEN LAVA_PASSWORD WORKER_TOKEN

echo '''
WORKER_NAME="--name laa-None"
URL="${LAVA_URL}"
TOKEN="--token ${WORKER_TOKEN}"
''' > /etc/lava-dispatcher-host/lava-docker-worker

# run tests
robot --pythonpath . --variable remote:"$IS_REMOTE" --outputdir=.. test/api/integration.robot
exit 0
