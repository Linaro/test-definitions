...
[ -n "${IP_ADDR}" ]
exit_on_fail "ethernet-ping-${INTERFACE}-state-UP" "ethernet-ping-${INTERFACE}-route"

# Get default Route IP address of a given interface
ROUTE_ADDR=$(ip route list  | grep default | awk '{print $3}')

# Run the test
for IP in "${IP_ADDR}" "${ROUTE_ADDR}" ; do
    run_test_case "ping ${IP} -c 5" "ethernet-ping-${INTERFACE}-${IP}"
done
