...
def parse_args():
    ...
    parser.add_argument('-r', '--result-file', dest='result_file', required=True, default='./result.json', help='Specify test result file.')
    parser.add_argument('-a', '--attachment', dest='attachment', help='Specify attachment file. Accepts quoted multiple files separated by a space.')
    parser.add_argument('-t', '--team', dest='team', default='erp', help='Team identifier. Defaults to "erp"')
    parser.add_argument('-p', '--project', dest='project', help='Project identifier. Defaults to the name of the linux distribution.')
    parser.add_argument('-b', '--build', dest='build', required=True, help='Build identifier.')
    parser.add_argument('-e', '--test-env', dest='test_env', help='Environmenr identitifer. Defaults to board name.')
    parser.add_argument('-u', '--url', dest='url', default='https://staging-qa-reports.linaro.org', help='Dashboard url. Defaults to https://staging-qa-reports.linaro.org.')
    parser.add_argument('-v', '--verbose', action='store_true', dest='verbose', default=True, help='Set log level.')
...
def squad_metadata(results):
    ...
    squad_metadata['test_version'] = results[-1]['version']
...
def main():
    ...
    if args.attachment is not None:
        for item in args.attachment.split():
    ...
    logger.info('Data to post: %s' % files)
