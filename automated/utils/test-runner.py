...
class TestPlan(object):
    ...
    def apply_overlay(self, test_list):
        ...
        if data.get('skip'):
            ...
            skip_paths = [test['path'] for test in skip_tests]
            for test in test_list:
                if test['path'] in skip_paths:
        ...
        if data.get('add'):
            add_tests = data['add']
            test_list_without_uuid = copy.deepcopy(test_list)
            for test in test_list_without_uuid:
                del test['uuid']
            for add_test in add_tests:
                if add_test in test_list_without_uuid:
