...
class RemoteTestRun(AutomatedTestRun):
    ...
    def copy_to_target(self):
        tc_relative_parent_dir = self.test['tc_relative_dir'].rsplit('/', 1)[0]
        create_target_dir = 'ssh %s "mkdir -p %s/%s"' % (self.args.target, self.test['target_test_path'], tc_relative_parent_dir)
        subprocess.call(shlex.split(create_target_dir))

        self.logger.info('Pushing test files to target %s' % self.args.target)
        self.__copy_file('%s/%s' % (self.test['test_path'], self.test['tc_relative_dir']), '%s/%s' % (self.test['target_test_path'], tc_relative_parent_dir))
        self.__copy_file('%s/automated/bin' % (self.test['test_path']), '%s/automated/' % (self.test['target_test_path']))
        self.__copy_file('%s/automated/lib' % (self.test['test_path']), '%s/automated/' % (self.test['target_test_path']))
        self.__copy_file('%s/automated/utils' % (self.test['test_path']), '%s/automated/' % (self.test['target_test_path']))
        self.__copy_file('%s/run.sh' % (self.test['test_path']), self.test['target_test_path'])
        self.__copy_file('%s/uuid' % (self.test['test_path']), self.test['target_test_path'])

    def run(self):
        self.copy_to_target()
        self.logger.info('Executing %s/run.sh remotely on %s' % (self.test['target_test_path'], self.args.target))
        shell_cmd = 'ssh %s "%s/run.sh 2>&1"' % (self.args.target, self.test['target_test_path'])
...
def main():
    ...
    if args.target:
        ...
        try:
            ...
            s.login(args.target.split('@')[1], args.target.split('@')[0])
            s.logout()
    ...
    for test in test_list:
        ...
        if args.target:
            target_user_home_cmd = 'ssh %s "echo $HOME"' % args.target
            target_user_home = subprocess.check_output(shlex.split(target_user_home_cmd)).strip()
