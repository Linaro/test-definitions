...
class RemoteTestRun(AutomatedTestRun):
    def copy_to_target(self):
        os.chdir(self.test['test_path'])
        tar_cmd = 'tar -caf target-test-files.tar run.sh uuid automated/lib automated/bin automated/utils %s' % self.test['tc_relative_dir']
        subprocess.call(shlex.split(tar_cmd))
        create_target_test_path_cmd = 'ssh %s "mkdir -p %s"' % (self.args.target, self.test['target_test_path'])
        subprocess.call(shlex.split(create_target_test_path_cmd))
        scp_cmd = 'scp ./target-test-files.tar %s:%s' % (self.args.target, self.test['target_test_path'])
        self.logger.info('Pushing test files to target with command: %s' % scp_cmd)
        subprocess.call(shlex.split(scp_cmd))
        uncompress_cmd = 'ssh %s "cd %s && tar -xf target-test-files.tar"' % (self.args.target, self.test['target_test_path'])
        self.logger.info('Uncompressing test files on target with command: %s' % uncompress_cmd)
        subprocess.call(shlex.split(uncompress_cmd))

    def run(self):
        self.copy_to_target()
        shell_cmd = 'ssh %s "%s/run.sh 2>&1"' % (self.args.target, self.test['target_test_path'])
        self.logger.info('Executing run.sh remotely on %s with command: %s' % (self.test['target_test_path'], self.args.target, shell_cmd))
...
def main():
    ...
    if args.target is not None:
        ...
        if pexpect.which('ssh') is None:
            ...
            sys.exit(1)
        target_user = args.target.split('@')[0]
        target_host = args.target.split('@')[1]
        s = pxssh.pxssh()
        try:
            s = pxssh.pxssh()
            s.login(target_host, target_user)
            s.logout()
    ...
    for test in test_list:
        ...
        if args.target is not None and args.target not in test['output']:
