    ...
    def test_list(self):
        if self.test_def:
            ...
            if 'tests' in test_plan:
                if 'automated' in test_plan['tests']:
                    test_list = test_plan['tests']['automated']
                    for test in test_list:
                        test['uuid'] = str(uuid4())
                else:
                    self.logger.error("'automated' field is missing from test plan")
                    sys.exit(1)
            else:
                self.logger.error("'tests' field is missing from test plan")
                sys.exit(1)
        else:
