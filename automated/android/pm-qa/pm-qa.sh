...
# If awk doesn't exsit on the target, replace it with 'buysbox awk'.
...
# glmark2 is requred to heat GPU. Install it if it is not installed.
if ! adb shell pm list packages | grep 'glmark2'; then
    # The following link isn't available publicly.
    # Please copy the apk to the currenty directory for local run.
