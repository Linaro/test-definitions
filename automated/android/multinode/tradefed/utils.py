...
class Device:
    ...
    def __init__(self, serial_or_address, logcat_output_filename, worker_job_id=None):
        self.serial_or_address = serial_or_address
        self.is_tcpip_device = bool(Device.tcpip_device_re.match(self.serial_or_address))
        self.logcat_output_file = open(logcat_output_filename, 'w')
        self.logcat = subprocess.Popen(['adb', '-s', serial_or_address, 'logcat'],
    ...
    def ensure_available(self, logger, timeout_secs=30):
        ...
        logger.debug('adb connection to %s lost! Trying to reconnect...' % self.serial_or_address)
        ...
        if not self.try_reconnect():
            logger.warning('adb connection to %s lost and reconnect failed!' % self.serial_or_address)
            return self._is_available
    ...
    def check_available(self, timeout_secs=30):
        return subprocess.run([
            ...
            'adb', '-s', self.serial_or_address, 'shell',
            'echo', '%s:' % self.serial_or_address, 'OK']).returncode == 0
    ...
    def try_reconnect(self, reconnectTimeoutSecs=60):
        ...
        if not self.is_tcpip_device:
            ...
            fastbootRebootTimeoutSecs = 10  # There is no point in waiting longer for fastboot
            subprocess.run(['timeout', str(fastbootRebootTimeoutSecs),
                            'fastboot', '-s', self.serial_or_address, 'reboot'])
            bootTimeoutSecs = max(10, int(reconnectTimeoutSecs) - fastbootRebootTimeoutSecs)
            return subprocess.run(
                ['sh', '-c', '. ../../../lib/sh-test-lib && . ../../../lib/android-test-lib && '
                    'export ANDROID_SERIAL="%s" && wait_boot_completed %s'
                    % (self.serial_or_address, bootTimeoutSecs)]).returncode == 0

        # adb may not yet have realized that the connection is broken
        subprocess.run(['adb', 'disconnect', self.serial_or_address])
        time.sleep(5)   # adb connect ~often~ fails when called ~directly~ after disconnect.

        if subprocess.run(['timeout', str(reconnectTimeoutSecs), 'adb', 'connect',
                           self.serial_or_address]).returncode != 0:
            return False
        if not self.check_available():
            return False
        # reestablish logcat connection
        self.logcat.kill()
        self.logcat = subprocess.Popen(['adb', '-s', self.serial_or_address, 'logcat'],
                                       stdout=self.logcat_output_file)
        return True
    ...
    def worker_handshake(self, command):
        ...
        """
        if (self.worker_job_id is None          # Nothing to do for local devices,
                or not Device.EXEC_IN_LAVA):    # and nothing to do when not called by LAVA.
            self.worker_handshake_iteration += 1
            return True
        ...
        subprocess.run(
            ['lava-send', 'master-sync-%s-%s' % (self.worker_job_id,
             str(self.worker_handshake_iteration)), 'command=%s' % command])
        if wait_for_acc:
            subprocess.run(
                ['lava-wait', "worker-sync-%s-%s"
...
class RetryCheck:
    ...
    def should_continue(self):
        return (self.current_retry < self.total_max_retries
...
class ResultSummary:
    ...
    def was_successful(self):
        return (self.failure_count == 0
                and self.modules_completed == self.modules_total)
