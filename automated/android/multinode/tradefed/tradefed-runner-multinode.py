...
parser.add_argument('-r', dest='RESULTS_FORMAT', required=False,
                    default=result_parser.AGGREGATED,
                    choices=[result_parser.AGGREGATED, result_parser.ATOMIC],
...
while child.isalive():
    ...
    for device in devices: device.ensure_available(logger=logger)
...
success = parser.parse_recursively(last_result_dir())
cleanup_and_exit(0 if success else 1,
    None if success else 'Failed to parse the TradeFed logs. Test result listing in the LAVA logs '
        'will be incomplete.')
