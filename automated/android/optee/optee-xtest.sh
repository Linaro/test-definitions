...
usage() {
    echo "Usage: $0 [-l <level>] [-t <test_suite>] [-s <android_serial>] [-b <boot_timeout>]" 1>&2
    exit 1
}

while getopts ":s:b:l:t:" o; do
  case "$o" in
...
check_cmd_existence "tee-supplicant"
adb shell "tee-supplicant &"
check_cmd_existence "xtest"
adb shell "xtest -l ${LEVEL} -t ${TEST_SUITE} 2>&1" | tee "${LOGFILE}"
