...
getTimeStampFromLogcat(){
    ...
    if [ -n "${key_line}" ]; then
        timestamp=$(echo "${key_line}" |awk '{printf "%s %s\n", $1, $2}')
        timestamp="${year}-${timestamp}"
        # the default date command does not recognize the --date option
        # so we use busybox date command here
        timestamp_sec=$(busybox date +%s --date="${timestamp}")
        timestamp_ms=$(echo "${timestamp}"|awk -F. '{print $2}')
        echo "${timestamp_sec}.${timestamp_ms}"
    fi
}
