...
class ApkRunnerImpl(ApkTestRunner):
    ...
    def parseResult(self):
        ...
        pat_score_unit = re.compile("^(?P<measurement>[\d\.]+)(?P<units>[^\d\.]+)$")
    ...
    def execute(self):
        ...
        if item:
            ...
            if item:
                ...
                time.sleep(3)
                self.dump_always()
                opt_expandableListView1 = self.vc.findViewByIdOrRaise("com.eembc.andebench:"
                                                                 "id/opt_expandable"
                                                                 "ListView1")
                if opt_expandableListView1:
                    for sub in opt_expandableListView1.children:
                        if not self.vc.findViewWithText(u'Memory', sub):
                            self.vc.findViewByIdOrRaise("com.eembc.andebench:id/cbx1",
                                                   sub).touch()
                            time.sleep(3)
                            self.dump_always()
        ...
        while True:
            try:
                self.dump_always()
                btn_start_on = self.vc.findViewByIdOrRaise("com.eembc.andebench:id/s1_runall")
                btn_start_on.touch()
                break
