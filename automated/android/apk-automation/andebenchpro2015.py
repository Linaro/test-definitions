...
class ApkRunnerImpl(ApkTestRunner):
    ...
    def parseResult(self):
        ...
        pat_score_unit = re.compile(
                            "^(?P<measurement>[\d\.]+)(?P<units>[^\d\.]+)$")
    ...
    def execute(self):
        ...
        if item:
            ...
            if item:
                ...
                time.sleep(3)
                self.dump_always()
                opt_expandableListView1 = self.vc.findViewByIdOrRaise(
                                                    "com.eembc.andebench:"
                                                    "id/opt_expandable"
                                                    "ListView1")
        ...
        while True:
            try:
                self.dump_always()
                btn_start_on = self.vc.findViewByIdOrRaise(
