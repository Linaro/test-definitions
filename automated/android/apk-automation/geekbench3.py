...
class ApkRunnerImpl(ApkTestRunner):
    ...
    def parseResult(self):
        raw_output_file = '%s/geekbench3-result.gb3' % self.config['output']
        self.logger.info('Pulling /data/user/0/com.primatelabs.geekbench3/files to output directory...')
        self.call_adb('pull /data/user/0/com.primatelabs.geekbench3/files %s' % self.config['output'])
        db_file_list = glob.glob('%s/*.gb3' % self.config['output'])
        if len(db_file_list) > 1:
            print('More then one db file found...')
            sys.exit(1)
