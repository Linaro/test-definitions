...
test01()
{
	...
	then
		...
		then
	        ...
	        ip link set eth0:1 mtu $ORI_MTUSZ >$LTPTMP/tst_ip.err 2>&1
		else
			tst_resm FAIL NULL \
...
test04()
{
    ...
    # Rearrange exec step.
    # Because "ip neigh show" will be STALE in very short time.
	ip neigh show 127.0.0.1 | head -n1 >$LTPTMP/tst_ip.out 2>&1 || RC_01=$?
	if [ $RC -ne 0 ]
	then
		tst_res TFAIL $LTPTMP/tst_ip.err \
