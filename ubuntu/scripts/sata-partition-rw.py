...
def sata_ext4_mount_umount():
    ...
    target_partition_name = device_name + "2"
    mount_point = "/media"
    ext4_mount = "sudo mount " + target_partition_name + " " + mount_point
    ext4_umount = "sudo umount " + mount_point

    # umount everything from mount point - cleaning
    umount_clean = "sudo umount " + mount_point
    ...
    if ext4_mount_return[0] != 0:
        ...
        commands.getstatusoutput("sudo mount > mount_log.txt 2>&1")
    ...
    if ext4_umount_return[0] != 0:
        ...
        commands.getstatusoutput("sudo mount > umount_log.txt 2>&1")
...
def sata_ext4_file_fill_edit():
    ...
    file_creation_1M = "sudo dd if=/dev/zero of=/media/file_1MB bs=4k count=256"
    ...
    if os.path.isfile(testfile_name) == True:
        os.system("sudo rm -rf " + testfile_name)
    file_creation_empty = "sudo touch " + testfile_name
    ...
    if file_creation_empty_return[0] != 0:
        ...
        os.system("sudo chmod 777 " + testfile_name)
...
def sata_ext4_dd_write_read():
    ...
    dd_write_1G = "sudo dd if=/dev/zero of=/media/file_1GB oflag=direct bs=4k count=262144 > dd_write_1GB_stdout.txt 2>&1"
    ...
    if os.path.isfile("/media/file_1GB") == False:
        ...
        dd_read_1G = "sudo dd if=/media/file_1GB iflag=direct of=/dev/null bs=4k count=262144 > dd_read_1GB_stdout.txt 2>&1"
    ...
    # umount and clean
    os.system("sudo rm -rf /media/file_1GB")
    umount_clean = "sudo umount " + mount_point
    commands.getstatusoutput(umount_clean)
    time.sleep(5)
    print "mount point cleaned!"
