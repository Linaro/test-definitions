...
test_session_xml_not_empty() {
    ...
    if [ ! -f /mnt/sdcard/session.xml ]; then
    fail_test "Unable to find /mnt/sdcard/session.xml"
    return 1
    fi

    session_file=`cat /mnt/sdcard/session.xml`
    if [ -z "$session_file" ]; then
        fail_test "Empty template session XML file at /mnt/sdcard/session.xml"
        return 1
    fi
...
test_gator_data_streaming_cmd() {
    ...
    /system/bin/gatord -s /mnt/sdcard/session.xml -o /mnt/sdcard/linaro-android-gator-data-streaming.apc
    if [ $? -ne 0 ]; then
...
test_gator_data_streaming_result() {
    ...
    if [ ! -d /mnt/sdcard/linaro-android-gator-data-streaming.apc ]; then
        fail_test "Gator data streaming result folder doesn't exist"
        return 1
    elif [ ! -f /mnt/sdcard/linaro-android-gator-data-streaming.apc/captured.xml ]; then
        fail_test "File captured.xml doesn't exist"
        return 1
    elif [ ! -s /mnt/sdcard/linaro-android-gator-data-streaming.apc/captured.xml ]; then
