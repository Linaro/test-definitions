...
pass_fail_parser() {
    ...
    cut -d "_" -f${field}- | sed 's/ /-/g' > ${TEST_SUITE}_result.txt
    ...
    do
        ...
        test_result=$(grep "^XTEST_TEE.*${test_id} [OK|FAILED]" \
                      ${TEST_SUITE}_output.txt | awk '{ print $2 }')
        if [ "$test_result" = "OK" ]; then
            sed "s/$line/$line pass/g" -i ${TEST_SUITE}_result.txt
            lava-test-case $test_case --result pass
        else
            sed "s/$line/$line fail/g" -i ${TEST_SUITE}_result.txt
...
benchmark_parser() {
    ...
    do
        ...
        print test_case"-"data_size" "speed; }' > ${test_id}_benchmark.txt
        while read line
        do
            test_case=$(echo $line | awk '{ print $1 }')
...
if [ $? -eq 0 ]; then
    lava-test-case xtest-run --result pass
else
    lava-test-case xtest-run --result fail
fi
