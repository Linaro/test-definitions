...
test_has_variable_external_storage() {
    ...
    #Add 5 minutes sleep time to avoid SD card partition is unavailable during the system boot
    sleep 300
...
test_write_on_sd_card() {
    ...
    echo "abcdefghijklmn" > $EXTERNAL_STORAGE/sd-mmc-test.txt
    ...
    if [ "$file_content" != "abcdefghijklmn" ]; then
