...
def validate_file(args, path):
    ...
    if path.endswith("yaml"):
        ...
        exitcode = exitcode + pep8_check(path, args.pep8_ignore)
    elif path.endswith("php"):
        exitcode = exitcode + validate_php(path)
    else:
        # try shellcheck by default
        exitcode = exitcode + validate_shell(path, args.shellcheck_ignore)
    return exitcode
...
def run_unit_tests(args, filelist=None):
    ...
    if filelist is not None:
        for filename in filelist:
            exitcode = exitcode + validate_file(args, filename)
    else:
        for root, dirs, files in os.walk('.'):
            if not root.startswith("./.git"):
                for name in files:
                    exitcode = exitcode + validate_file(
                        args,
                        root + "/" + name)
    return exitcode
